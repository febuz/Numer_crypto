...
from utils.memory_utils import check_memory_available, get_memory_usage, clear_memory
from utils.feature.engineering.timeseries import create_rolling_features
...
class PolarsFeatureGenerator:
    ...
    def __init__(self, output_dir: str = "/media/knight2/EDB/numer_crypto_temp/data/features", 
        ...
        if self.enable_gpu:
            try:
                ...
                self.gpu_accelerator = PolarsGPUAccelerator(
                    force_gpu=True,
                    use_all_gpus=True
                )
                logger.info("GPU acceleration enabled")
                
                # Set mixed precision environment
                if self.enable_mixed_precision:
                    os.environ["TORCH_ALLOW_TF32_CUBLAS_OVERRIDE"] = "1"
                    os.environ["TORCH_ALLOW_TF32_CUCONV_OVERRIDE"] = "1"
                    logger.info("Mixed precision enabled")
                    
            except Exception as e:
                logger.warning(f"GPU acceleration failed to initialize: {e}")
                self.gpu_accelerator = None
        
        logger.info(f"Polars Feature Generator initialized with max_features={max_features}")
        logger.info(f"Output directory: {self.output_dir}")
        logger.info(f"GPU acceleration: {'enabled' if self.gpu_accelerator else 'disabled'}")
    ...
    def generate_features(self, input_file: str, feature_modules: Optional[List[str]] = None) -> str:
        ...
        # Check memory
        if not check_memory_available(min_available_gb=2.0):
        ...
        try:
            ...
            # GPU acceleration strategy selection
            estimated_size_gb = (df.height * df.width * 8) / (1024**3)
            
            # Try GPU acceleration first if available and data is suitable
            if (self.gpu_accelerator and 
                estimated_size_gb > 0.5 and  # Minimum size for GPU benefit
                estimated_size_gb < 20.0):   # Maximum size for single-node processing
                
                logger.info(f"Attempting GPU-accelerated feature generation (data size: {estimated_size_gb:.2f} GB)")
                try:
                    # Create progress tracking
                    unique_symbols = df.select(pl.col(self.group_col)).n_unique()
                    logger.info(f"Processing {unique_symbols} symbols with GPU acceleration")
                    
                    # Use GPU accelerator for feature generation
                    result_df = self.gpu_accelerator.generate_all_features(
                        df=df,
                    ...
                    # Check if GPU acceleration was successful
                    if result_df is not None and result_df.width > df.width:
                        feature_count = result_df.width - df.width
                        logger.info(f"GPU acceleration successful: generated {feature_count} features")
                        df = result_df
                        
                        # Skip CPU processing
                        use_chunks = False
                    else:
                        logger.warning("GPU acceleration returned no features, falling back to CPU")
                ...
                if self.gpu_accelerator:
                    if estimated_size_gb <= 0.5:
                        logger.info(f"Data too small for GPU acceleration ({estimated_size_gb:.2f} GB)")
                    elif estimated_size_gb >= 20.0:
                        logger.info(f"Data too large for single-node GPU processing ({estimated_size_gb:.2f} GB)")
            ...
            if use_chunks:
                ...
                clear_memory(full_gc=True, verbose=False)
            ...
            for agg_fn in ["mean", "std", "max", "min"]:
                ...
                for i in range(0, len(numeric_cols), batch_size):
                    ...
                    for col in batch_cols:
                        ...
                        try:
                            # Newer versions use min_samples
                            if agg_fn == "mean":
                                expr = pl.col(col).rolling_mean(window, min_samples=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "std":
                                expr = pl.col(col).rolling_std(window, min_samples=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "max":
                                expr = pl.col(col).rolling_max(window, min_samples=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "min":
                                expr = pl.col(col).rolling_min(window, min_samples=1).alias(f"{col}{fn_suffix}")
                        except TypeError:
                            # Older versions use min_periods
                            if agg_fn == "mean":
                                expr = pl.col(col).rolling_mean(window, min_periods=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "std":
                                expr = pl.col(col).rolling_std(window, min_periods=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "max":
                                expr = pl.col(col).rolling_max(window, min_periods=1).alias(f"{col}{fn_suffix}")
                            elif agg_fn == "min":
                                expr = pl.col(col).rolling_min(window, min_periods=1).alias(f"{col}{fn_suffix}")
                        exprs.append(expr)
                    ...
                    for col_idx in range(0, len(numeric_cols), cols_per_batch):
                        ...
                        try:
                            ...
                            try:
                                ...
                                clear_memory(full_gc=should_full_gc, verbose=verbose_log)
                                ...
                                if len(batch_cols) > 1:
                                    ...
                                    for col in batch_cols:
                                        try:
                                            # Build expression for single column
                                            if agg_fn == "mean":
                                                try:
                                                    expr = pl.col(col).rolling_mean(window, min_samples=1).alias(f"{col}{fn_suffix}")
                                                except TypeError:
                                            ...
                                            if individual_success % 10 == 0:
                                                clear_memory(full_gc=False, verbose=False)
    ...
    def _generate_lag_features(self, df: pl.DataFrame, numeric_cols: List[str]) -> pl.DataFrame:
        ...
        for i in range(0, len(numeric_cols), batch_size):
            ...
            for lag in self.lag_periods:
                ...
                batch_exprs = []
                for col in batch_cols:
                    batch_exprs.append(pl.col(col).shift(lag).alias(f"{col}_lag_{lag}"))
            ...
            for i in range(0, len(limited_cols), batch_size):
                ...
                for col in batch_cols:
                    ...
                    try:
                        ...
                        del ewm_col
                        # Only do full GC periodically
                        clear_memory(full_gc=(features_added % 50 == 0))
            ...
            for col2 in base_cols[i+1:]:
                ...
                try:
                    ...
                    try:
                        ...
                        result_df = result_df.with_columns([(f"{col1}_X_{col2}", pl.col(col1) * pl.col(col2))])
                ...
                if "price" in col1 and "price" in col2:
                    try:
                        # Try the newest polars API first
                        result_df = result_df.with_columns(
                            (pl.col(col1) / pl.col(col2).clip_min(0.0001)).alias(f"{col1}_DIV_{col2}")
                        )
                    except Exception as e1:
                        ...
                        try:
                            ...
                            # Fall back to most compatible version
                            result_df = result_df.with_columns([(f"{col1}_DIV_{col2}", pl.col(col1) / pl.col(col2).clip_min(0.0001))])
    ...
    def _generate_technical_indicators(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            try:
                # Try the newest polars API first
                result_df = result_df.with_columns(
                    (pl.col(col_map['close']) - pl.col(col_map['open'])).alias('price_change')
                )
            except Exception as e1:
                ...
                try:
                    ...
                    # Fall back to most compatible version
                    result_df = result_df.with_columns([('price_change', pl.col(col_map['close']) - pl.col(col_map['open']))])
            
            # 2. Price range
            try:
                # Try the newest polars API first
                result_df = result_df.with_columns(
                    (pl.col(col_map['high']) - pl.col(col_map['low'])).alias('price_range')
                )
            except Exception as e1:
                ...
                try:
                    ...
                    # Fall back to most compatible version
                    result_df = result_df.with_columns([('price_range', pl.col(col_map['high']) - pl.col(col_map['low']))])
            
            # 3. Volume * price change (approximation of volume delta)
            try:
                # Try the newest polars API first
                result_df = result_df.with_columns(
                    (pl.col(col_map['volume']) * (pl.col(col_map['close']) - pl.col(col_map['open']))).alias('volume_price_change')
                )
            except Exception as e1:
                ...
                try:
                    ...
                    # Fall back to most compatible version
                    result_df = result_df.with_columns([('volume_price_change', pl.col(col_map['volume']) * (pl.col(col_map['close']) - pl.col(col_map['open'])))])
